...
# ─── Compile-only test ──────────────────────────────────────────────────
# Admin is hardcoded to the deploy wallet (the same key the off-chain
# oracle worker runs with). Without this, sp.test_account("admin") would
# bake a deterministic-but-unrelated tz1 address into initial storage,
# and the worker would skip all squares actions with "not authorised".
@sp.add_test()
def test():
