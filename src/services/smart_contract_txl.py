...
def main():
    class TxlManager(sp.Contract):
        ...
        def updateOwner(self, params):
            '''
           Listen for sales with oracle update by admin/oracle
            '''
            if sp.sender == self.data.oracle:
                sp.cast(params.txlId, sp.int_or_nat)
                sp.cast(params.address, sp.address)
                self.data.idLookUp[params.txlId].owner = params.address
            else:
                sp.emit('not Oracle Error', tag='notOracle')


        @sp.entrypoint
        def payTxlHolder(self):
            '''
            '''    
            ...
            if isOwner == 1:
                ...
                sp.emit('notTXLOwner', tag='notTXLOwner')
